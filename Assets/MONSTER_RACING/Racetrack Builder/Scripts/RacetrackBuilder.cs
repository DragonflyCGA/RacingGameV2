...
public static class RacetrackBuilder
{
        ...
        RacetrackMeshInfoCache meshCache)
    {
        ...
        try
        {
            ...
            foreach (var p in templateCopyParams)
                CreateMeshTemplateCopy(p, meshCache, generateSecondaryUVs, worldFromTrack);
        ...
        out string msg)
    {
        ...
        try
        {
            ...
            foreach (var p in templateCopyParams)
            {
                ...
                int hash = p.CalcHash(new Hasher());
                int spacingGroupsHash = p.CalcSpacingGroupsHash(new Hasher());
    ...
    Matrix4x4 worldFromTrack)
    {
        return CreateMeshTemplateCopy(p, meshCache, generateSecondaryUVs, worldFromTrack, p.CalcHash(new Hasher()), p.CalcSpacingGroupsHash(new Hasher()));
    }
    ...
    private static void WarpMeshToCurves(
        Mesh mesh,
